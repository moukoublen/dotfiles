...
_marmalade_ps1() {
  ...
  local F_GIT='\[\033[0;49;93m\]'
  local F_MAI='\[\033[0;49;36m\]'
  local F_RST='\[\033[0;49;39m\]'

  local p_st1="${F_MAI}╭${F_RST}"
  local p_st2="${F_MAI}╰${F_RST}"
  local p_sep="${C_SEP}|${F_RST}"
  local p_gfr="${p_sep} ${F_GIT}%s${F_RST}" #__git_ps1 format string
  local p_git="\$(declare -F __git_ps1 &>/dev/null && __git_ps1 \"${p_gfr}\")"
  local p_pwd="${F_PTH}\w${F_RST}"
  local p_usr="${F_MAI}\u@\h${F_RST}"

  local ps1_line1="${p_st1} ${p_usr} ${p_sep} ${p_pwd} ${p_git}"
